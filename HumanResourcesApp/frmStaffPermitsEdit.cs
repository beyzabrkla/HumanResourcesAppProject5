...
{
    public partial class frmStaffPermitsEdit : Form
    {
        Data mydata = new Data();

        public int id { get; set; }
        public frmStaffPermitsEdit()
        {
            InitializeComponent();
        }
        ...
        private void frmStaffPermitsAdd_Load(object sender, EventArgs e)
        {
            ...
            ret = mydata.getPermissionById(id);
            if (ret.ReturnCode == enums.ReturnCode.Ok)
            {
                DBPermit permits = (DBPermit)ret.objectData;
                dpdate.Value = permits.day;
                richTextBox1.Text = permits.reasonForPermission;
                comboBox_Personals.SelectedValue = permits.userId.ToString();
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                ReturnObject ret = mydata.EditPermission(permit);
                if (ret.ReturnCode == enums.ReturnCode.Ok)
                {
                    MessageBox.Show(ret.message);
                    this.Close();
